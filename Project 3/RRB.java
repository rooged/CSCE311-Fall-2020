...
{	
    ...
    */
    ...
    {
		//create resource variable of the 
		ResourceCB resource = this.getResource();

		//decreases number of available instances by the requested amount [set(available - quantity)]
        resource.setAvailable(resource.getAvailable() - this.getQuantity());

		//increases number of alloted instances by requested amount [set(thread, allocated + quantity)]
		resource.setAllocated(this.getThread(), resource.getAllocated(this.getThread()) + this.getQuantity());
